...
namespace Microsoft.Health.Expressions
{
    public class SubtractFunctionTests
    {
        ...
        public void Integer_Subtraction_Succeeds(long leftOperand, long rightOperand, long result)
        {
        ...
        public void Double_Subtraction_Succeeds(double leftOperand, double rightOperand, double result)
        {
            var data = JObject.FromObject(new
            {
                left = leftOperand,
        ...
        [Theory]
        [InlineData(0, 0, 0.0)]
        [InlineData(-1, 100.0, -101.0)]
        [InlineData(1, 10.5, -9.5)]
        [InlineData(0, double.MaxValue, double.MinValue)]
        [InlineData(0, double.MinValue, double.MaxValue)]
        public void Mixed_Subtraction_Succeeds(long leftOperand, double rightOperand, double result)
        {
            var data = JObject.FromObject(new
            {
                left = leftOperand,
        ...
        {
            Assert.Throws<Exception>(() => _jmesPath.Parse("subtract()"));
            Assert.Throws<Exception>(() => _jmesPath.Parse("subtract(left"));
            Assert.Throws<Exception>(() => _jmesPath.Parse("subtract(left, right, up"));
        }
    }
}
