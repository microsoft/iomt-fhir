...
az acr build --registry $containerRegistry --image "$($normalizationImage):$($imageTag)" --file .\src\console\Microsoft.Health.Fhir.Ingest.Console.Normalization\Dockerfile .

Write-Host "Building FHIR Transformation image..."
az acr build --registry $containerRegistry --image "$($fhirTransformationImage):$($imageTag)" --file .\src\console\Microsoft.Health.Fhir.Ingest.Console.FhirTransformation\Dockerfile .