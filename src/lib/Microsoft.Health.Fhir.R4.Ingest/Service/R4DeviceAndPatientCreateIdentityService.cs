...
namespace Microsoft.Health.Fhir.Ingest.Service
{
    ...
    {
        public R4DeviceAndPatientCreateIdentityService(ResourceManagementService resourceIdService)
            : base(resourceIdService, ResourceIdentityServiceType.Create)
        {
        }
        ...
        protected async virtual Task<(string DeviceId, string PatientId)> EnsureDeviceAndPatientExistsAsync(IMeasurementGroup input)
        {
            ...
            if (device.Patient == null)
            {
                ...
                device = await ResourceManagementService.FhirServerRepository.UpdateResourceAsync(device, true).ConfigureAwait(false);
            }
