...
{
    public class PowerFunction : JmesPathFunction
    {
        ...
        public override JToken Execute(params JmesPathFunctionArgument[] args)
        {
            ...
            {
                return new JValue(Math.Pow(args[0].Token.Value<double>(), args[1].Token.Value<double>()));
            }
            else
            {
                return new JValue(Convert.ToInt64(Math.Pow(args[0].Token.Value<long>(), args[1].Token.Value<long>())));
            }
        }
    }
}
